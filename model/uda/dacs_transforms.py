...
def strong_transform(param, data=None, target=None):
    ...
    print("std min:", torch.min(param['std']))
    data, target = one_mix(mask=param['mix'], data=data, target=target)
    data, target = color_jitter(color_jitter=param['color_jitter'], s=param['color_jitter_s'], p=param['color_jitter_p'], mean=param['mean'], std=param['std'], data=data, target=target)
    data, target = gaussian_blur(blur=param['blur'], data=data, target=target)
    return data, target


def get_mean_std(img_metas, dev):
    mean = [torch.as_tensor(img_metas[i]['img_norm_cfg']['mean'], device=dev) for i in range(len(img_metas))]
    mean = torch.stack(mean).view(-1, 3, 1, 1)
    std = [torch.as_tensor(img_metas[i]['img_norm_cfg']['std'], device=dev) for i in range(len(img_metas))]
...
def renorm_(img, mean, std):
    ...
    img.mul_(255.0).sub_(mean).div_(std + 1e-6)  # 数值稳定性： 在进行除法运算时，添加一个小的 epsilon 值，以避免除以零的情况
...
def color_jitter(color_jitter, mean, std, data=None, target=None, s=.25, p=.2):
    ...
    # 当i =1时，Contains NaN: color_jitter:data_Yes
    # Number of NaNs in color_jitter:data: 477
    check_nan(data, "color_jitter:data")
    check_nan(target, "color_jitter:target")
    return data, target
    ...
    if not (data is None):
        if data.shape[1] == 3:
            if blur > 0.5:
                ...
                data = seq(data)
    # 当i =1时，Contains NaN: gaussian_blur:data_Yes
    # Number of NaNs in gaussian_blur:data: 113979
    check_nan(data, "gaussian_blur:data")
    check_nan(target, "gaussian_blur:target")
    return data, target
...
def one_mix(mask, data=None, target=None):
    ...
    check_nan(data, "one_mix:data")
    check_nan(target, "one_mix:target")
    return data, target

def check_nan(img, img_name=""):
    if img is not None:
